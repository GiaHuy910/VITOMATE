...
# Đọc các biến bắt buộc từ môi trường (Đồng bộ với Builder)
...
if [ "$IS_UPDATE" = "true" ]; then
    ...
    echo "===> [2/5] Cấu hình Docker Insecure Registry (${REGISTRY_URL})..."
    mkdir -p /etc/docker
...
# =========================================================================
# BẮT ĐẦU ĐỒNG BỘ CODE VÀ KHỞI ĐỘNG LẠI AGENT (Chạy cho cả Bootstrap & Update)
# =========================================================================

echo "===> [4/5] Cấu hình thư mục chứa Agent và Service..."

# 🟢 KHÔNG dùng rm -rf "${INSTALL_DIR}" để tránh mất dữ liệu apps/
mkdir -p "${INSTALL_DIR}/apps"

# Đồng bộ source code Agent từ /tmp/agent sang /opt/agent (ghi đè file cũ, giữ lại folder apps/)
if [ -d "/tmp/agent" ]; then
    cp -r /tmp/agent/. "${INSTALL_DIR}/"
fi

echo "===> Tạo/Cập nhật file cấu hình Môi trường Agent..."
...
echo "===> SETUP HOÀN TẤT! Deploy Agent [${WORKER_ID}] (Update Mode: ${IS_UPDATE}) đã sẵn sàng."